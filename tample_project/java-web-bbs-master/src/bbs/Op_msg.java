...
public class Op_msg  implements java.io.Serializable{
	...
	//列出板块内所有文章
	public java.util.ArrayList<ForumBean> showmsg(javax.servlet.http.HttpServletRequest request)
		throws Exception{
		if(request.getParameter("boardID")==null){
				return null;
		}
		sqlStr = "select * from forumInfo where boardID =? and PID=0 order by flag desc, msgID desc";
		java.util.ArrayList<ForumBean> al = new java.util.ArrayList<ForumBean>();
		DataBase db = new DataBase();
		conn = db.getconn();
		pstmt = conn.prepareStatement(sqlStr);
	...
	//查询
	public java.util.ArrayList<ForumBean> search(javax.servlet.http.HttpServletRequest request)
		throws Exception{
	...
	//列出文章中所有的帖子
	public java.util.ArrayList<ForumBean> showAticle(javax.servlet.http.HttpServletRequest request)
		throws Exception {
