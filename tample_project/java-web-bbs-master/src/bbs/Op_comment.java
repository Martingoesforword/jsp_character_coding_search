...
public class Op_comment  implements java.io.Serializable{
	...
	//列出所有留言
	public java.util.ArrayList<CommentBean> showcomment(javax.servlet.http.HttpServletRequest request)
		throws Exception{
		//if(request.getParameter("boardID")==null){
		//		return null;
		//}
		sqlStr = "select * from commentinfo";
		java.util.ArrayList<CommentBean> al = new java.util.ArrayList<CommentBean>();
		DataBase db = new DataBase();
		conn = db.getconn();
		pstmt = conn.prepareStatement(sqlStr);
